...
и предлагаем её выбрать в компоненте графа
...
теперь сделаем граф трёхмерным

в шейдере добавляем зависимость цвета от положения в пространстве
...
в функциях добавляем зависимость от z
...
в компоненте делаем больше точек и расставляем их рядами вдоль оси z
...
![image](https://user-images.githubusercontent.com/43134602/153744720-63fdf90a-510f-4779-8550-a715bb906323.png)

эти графы всё ещё недостаточно трёхмерные e_e

по сути, тут нельзя построить вертикальную линию, либо сделать так, чтоб граф выгнулся через верх назад...

будем возвращать не только \\{y\\}, а все \\{x,y,z\\}
...
помняем [сигнатуру](https://ru.wikipedia.org/wiki/API#%D0%A1%D0%B8%D0%B3%D0%BD%D0%B0%D1%82%D1%83%D1%80%D0%B0_%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8) функций, добавив пару новых
...
public static class FuncLibrary
{
  ...
  {
    float r = Cos(0.5f * PI * v);
    return new Vector3(r * Sin(PI * (u + 0.2f * t)), Sin(0.5f * PI * v), r * Cos(PI * (u + 0.2f * t)));
  }
...
теперь положение точек на плоскости \\{XZ\\} может меняться, поэтому будем пересчитывать `x` и `z`, вернее `u` и `v` в методе `Update`
